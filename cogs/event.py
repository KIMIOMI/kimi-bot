...
class 이벤트(commands.Cog):
    ...
    async def 이벤트(self, ctx, name: str, time):
        ...
        try:
            if name == "지진":
                ...
                await msg_channel.send(f"{ctx.guild.get_role(922275420764770334).mention} 지진이 발생했습니다! 지진이 발생했습니다!!")
                await asyncio.sleep(60 * time)
                event.rob_event = False
                await msg_channel.send(f"{ctx.guild.get_role(922275420764770334).mention} 지진이 멈췄습니다!")
            elif name == "보스":
                if time > 60:
                ...
                await msg_channel.send(f"{ctx.guild.get_role(922275420764770334).mention} 보스가 등장 했습니다. 보스가 등장했습니다!! 보스 이름 {event.boss['name']}")
                await asyncio.sleep(60 * time)
                event.isboss = False
                event.boss = ''
                await msg_channel.send(f"{ctx.guild.get_role(922275420764770334).mention} 보스 사냥을 완료 하였습니다. 모든 기여도 정산이 완료 되었습니다.")
            else:
