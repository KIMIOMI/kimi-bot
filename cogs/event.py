...
class 이벤트(commands.Cog):
    ...
    async def 이벤트(self, ctx, name: str):
        """ 이벤트를 발생시킵니다. (관리자용) (!이벤트 [이벤트명]) """
        try:
            if name == "지진":
                event.rob_event = True
                msg_channel = ctx.guild.get_channel(db.channel_data["주막"])
                await msg_channel.send(f"{ctx.guild.get_role(973444672754171944).mention} 지진이 발생했습니다! 지진이 발생했습니다!!")
                await asyncio.sleep(60)
                event.rob_event = False
                await msg_channel.send(f"{ctx.guild.get_role(973444672754171944).mention} 지진이 멈췄습니다!")
            elif name == "보스":
                if event.isboss:
                ...
                await msg_channel.send(f"{ctx.guild.get_role(973444672754171944).mention} 보스가 등장 했습니다. 보스가 등장했습니다!! 보스 이름 {event.boss['name']}")
                await asyncio.sleep(60)
                event.isboss = False
                event.boss = ''
                await msg_channel.send(f"{ctx.guild.get_role(973444672754171944).mention} 보스 사냥을 완료 하였습니다. 모든 기여도 정산이 완료 되었습니다.")
            else:
